...
export async function auditRemoteLists({ execute = false, client = new TmdbClient() } = {}) {
  ...
  const results = await mapLimit(rails, 16, async (rail) => {
    const prior = state.rails[rail.key];
    try {
      if (!prior?.listId) throw new Error("missing managed list ID");
      const readback = await client.listV3All(prior.listId);
      ...
      const confirmation = identities((await client.listV3All(prior.listId)).items ?? []);
      ...
      for (const identity of removed) {
        ...
        try { await client.details(type, Number(rawId)); return { key: rail.key, listId: prior.listId, status: "failed", error: `remote item disappeared but TMDB details still exists: ${identity}`, added, removed }; }
  ...
  const report = { version: 1, date: today, mode: execute ? "execute" : "dry-run", totals: { considered: results.length, valid: results.filter((item) => item.status === "valid").length, repairedDeleted: execute && !failed.length ? repairable.length : 0, repairableDeleted: repairable.length, failed: failed.length }, results: reportResults };
